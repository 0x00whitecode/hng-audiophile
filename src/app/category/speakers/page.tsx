...
export default function SpeakersPage() {
  const speakers = useMemo(() => products.filter((p) => p.category === "speakers"), []);

  // Filtering/sorting state
  const [sort, setSort] = useState<SortKey>("featured");
  const [filterZX9, setFilterZX9] = useState(true);
  const [filterZX7, setFilterZX7] = useState(true);
  ...
  const visible = useMemo(() => {
    ...
    // Series filters
    list = list.filter((p) => {
      const isZX9 = /ZX9/i.test(p.name);
      const isZX7 = /ZX7/i.test(p.name);
      if (isZX9 && !filterZX9) return false;
      if (isZX7 && !filterZX7) return false;
      if (!isZX9 && !isZX7) return true; // future series
      return true;
    });
    // Price range
    ...
    list.sort((a, b) => {
      switch (sort) {
        ...
        case "featured":
        default: {
          // Keep ZX9 ahead of ZX7 to mirror design ordering
          const rank = (id: string) => (id.toLowerCase().includes("zx9") ? 0 : id.toLowerCase().includes("zx7") ? 1 : 2);
          return rank(a.id) - rank(b.id);
        }
      }
    });
    return list;
  }, [speakers, sort, filterZX9, filterZX7, minPrice, maxPrice]);
    ...
    <div className="container py-10">
      ...
      <nav aria-label="Breadcrumb" className="text-sm text-neutral-700">
        <ol className="flex items-center gap-2">
          ...
          <li aria-current="page" className="font-medium">Speakers</li>
        </ol>
      </nav>

      {/* Header */}
      <header className="mt-4">
        <h1 className="text-3xl font-semibold uppercase tracking-widest">Speakers</h1>
        <p className="mt-2 text-neutral-700 max-w-2xl">Explore premium speakers engineered for clear, room‑filling sound. Filter and sort to find the right model for your setup.</p>
      </header>
      ...
      <section className="mt-8 rounded border p-4" aria-label="Filtering and sorting">
        <div className="grid md:grid-cols-3 gap-4">
          ...
          <fieldset className="space-y-2" aria-label="Series">
            <legend className="text-sm">Filter series</legend>
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" checked={filterZX9} onChange={(e) => setFilterZX9(e.target.checked)} /> ZX9
            </label>
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" checked={filterZX7} onChange={(e) => setFilterZX7(e.target.checked)} /> ZX7
            </label>
          </fieldset>
      ...
      {/* Product Listing */}
      <section className="mt-8 grid md:grid-cols-2 lg:grid-cols-3 gap-6" aria-label="Speakers product list">
        {visible.map((p) => (
        ...
        {visible.length === 0 && (
          <p className="text-neutral-700">No speakers match your filters.</p>
        )}
      </section>
