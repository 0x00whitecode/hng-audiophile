...
export default function EarphonesPage() {
  const earphones = useMemo(() => products.filter((p) => p.category === "earphones"), []);

  // Filtering/sorting state
  const [sort, setSort] = useState<SortKey>("featured");
  const [filterYX1, setFilterYX1] = useState(true);
  const [minPrice, setMinPrice] = useState<string>("");
  const [maxPrice, setMaxPrice] = useState<string>("");

  const visible = useMemo(() => {
    let list = [...earphones];
    // Model filter (prepared for future models)
    list = list.filter((p) => {
      const isYX1 = /YX1/i.test(p.name);
      if (isYX1 && !filterYX1) return false;
      return true;
    });
    // Price range
    ...
    return list;
  }, [earphones, sort, filterYX1, minPrice, maxPrice]);
    ...
    <div className="container py-10">
      ...
      <nav aria-label="Breadcrumb" className="text-sm text-neutral-700">
        <ol className="flex items-center gap-2">
          ...
          <li aria-current="page" className="font-medium">Earphones</li>
        </ol>
      </nav>

      {/* Header */}
      <header className="mt-4">
        <h1 className="text-3xl font-semibold uppercase tracking-widest">Earphones</h1>
        <p className="mt-2 text-neutral-700 max-w-2xl">Compact and comfortable earphones for refined audio clarity. Filter and sort to quickly find what fits your lifestyle.</p>
      </header>
        ...
        <div className="grid md:grid-cols-3 gap-4">
          ...
          <fieldset className="space-y-2" aria-label="Model">
            ...
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" checked={filterYX1} onChange={(e) => setFilterYX1(e.target.checked)} /> YX1
            </label>
          </fieldset>
      ...
      {/* Product Listing */}
      <section className="mt-8 grid md:grid-cols-2 lg:grid-cols-3 gap-6" aria-label="Earphones product list">
        {visible.map((p) => (
        ...
        {visible.length === 0 && (
          <p className="text-neutral-700">No earphones match your filters.</p>
        )}
      </section>
